...
The first part of property~\eqref{eq:nextwork} follows because $r \in C'$. The second part assumes that $s$ is the point from which the server that moved to service $r$, and follows from the definition of the WFA.
...
%hiphan for k-competitive
% $k$-server
% WHen using ALG, OPT, MIN... mathsc (math small caps), or \text. Want it to render all as one workd, not as individaul variables. TEC as well.